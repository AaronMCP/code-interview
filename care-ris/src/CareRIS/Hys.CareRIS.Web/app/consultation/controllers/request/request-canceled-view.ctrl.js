consultationModule.controller('RequestCancelledController', ['$log', '$scope', 'consultationService', '$stateParams',
    '$state', '$modal', 'enums', 'requestService',
    function ($log, $scope, consultationService, $stateParams, $state, $modal, enums, requestService) {
        'use strict';
        $log.debug('RequestCancelledController.ctor()...');
        ...
        (function () {
            ...
            if ($stateParams.requestId) {
                consultationService.getConsultationDetailAsync($stateParams.requestId).success(function (data) {
                    $scope.report = requestService.getBaseRequestInfo(data, $scope.enums.consultationRequestStatus.Cancelled);

                    $scope.report.reasonType = enums.consultationDictionaryType.cancaleReason;
