<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1" xmlns:ris="urn:hl7-org:v2xml" xmlns:csb="http://www.carestream.com/csbroker">
    ...
    -->
			<xsl:for-each select="//Body/*">
				<xsl:choose>
					...
					<xsl:when test="name()= 'SIU_S12'">
						<csb:Order>
							<xsl:call-template name="SIU_S12"/>
						</csb:Order>
					</xsl:when>
					<xsl:when test="name()= 'SIU_S13'">
						<csb:Order>
							<xsl:call-template name="SIU_S12"/>
						</csb:Order>
					</xsl:when>
					<xsl:when test="name()= 'SIU_S15'">
						<csb:Order>
							<xsl:call-template name="SIU_S12"/>
						</csb:Order>
					</xsl:when>
					...
					<xsl:when test="name()= 'ACK' or name()= 'ORR_O02'">
						<csb:Index>
	...
	<xsl:template name="SIU_S12">
		<xsl:for-each select="SCH">
			<csb:table>
				<xsl:call-template name="DATAINDEX"/>
				
				<xsl:call-template name="PATIENT"/>
				
				<xsl:call-template name="ORDER_SIU"/>
			</csb:table>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template name="ACK">
		<csb:table>
			<xsl:call-template name="DATAINDEX_ACK"/>
		</csb:table>
	</xsl:template>
	<xsl:include href="./DATAINDEX.xsl"/>
	<xsl:include href="./DATAINDEX_ACK.xsl"/>
	<xsl:include href="./PATIENT.xsl"/>
	<xsl:include href="./ORDER.xsl"/>
	<xsl:include href="./ORDER_SIU.xsl"/>
