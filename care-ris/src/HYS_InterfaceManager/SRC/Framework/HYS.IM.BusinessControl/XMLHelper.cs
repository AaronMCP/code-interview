...
namespace HYS.IM.BusinessControl
{
  public  class XMLHelper
    ...
    /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
      ...
      public static DataTable XmlToDataTable(string xmlFile,string xpath)
      {
          ...
          XmlNode root = GetXmlNodeByXpath(xmlFile, xpath);

          foreach (XmlNode xn in root.ChildNodes)
          {
              if(dt.Columns.Contains(xn.Name)==false)
              {
              dt.Columns.Add(new DataColumn(xn.Name, typeof(string)));
              }
          }

          //Console.WriteLine(dt.Columns.Count + "aaaaaaaaaaaaaa"); 

          foreach (XmlNode xn2 in root.ChildNodes)
          {

              DataRow dr = dt.NewRow();


                  dr[xn2.Name] = xn2.InnerText;
     ...
     /// <summary>
     /// 依据匹配XPath表达式的所有节点来判断其下属的子节点中是否已含有指定属性和属性值的元素
     /// </summary>
     /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
     /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名</param>
     /// <param name="xmlAttributeName">要匹配xmlAttributeName的属性名称</param>
     /// <param name="value">属性值</param>
     /// <returns>有返回true，没返回false</returns>
     public static bool VerifyHasAttributeValueUnderXpathOrNot(string xmlFileName, string xpath, string xmlAttributeName, string value,string nodeValue) 
     {
        bool isExistsAttribute = false;//标识属性是否存在
         XmlDocument xmlDoc = new XmlDocument();
         try
                 ...
                 foreach (XmlNode node in xmlNodes)
                 {
                     ...
                     if (node != null)
                     {
                         ...
                         foreach (XmlAttribute attribute in node.Attributes)
                         {
                             ...
                             if (attribute.Name.ToLower() == xmlAttributeName.ToLower() && attribute.InnerText.Trim().Equals(value))
                             {
                     ...
                     if (node.InnerText.ToLower().Split(',')[0].Equals(nodeValue.ToLower().Split(',')[0]))
                     {

                         matchedTagValueCount++;
                     }

                     if (matchedAttributeCount > 0 && matchedTagValueCount > 0) 
                     {
         ...
         return isExistsAttribute;
     
     }

     /// <summary>
     /// CR5 医生在断网情况下，先新建文件，后删除该文件，再新建后修改文件，
     /// 最后新建的同名文件及内容需要在断点续传时得以展现等多次操作MERGE问题
     /// 依据匹配XPath表达式的所有节点来判断其下属的子节点中是否已含有指定属性和属性值的元素,然后更新节点的值
     /// </summary>
     /// <param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
     /// <param name="xpath">要匹配的XPath表达式(例如:"//节点名//子节点名</param>
     /// <param name="xmlAttributeName">要匹配xmlAttributeName的属性名称</param>
     /// <param name="value">属性值</param>
     /// <param name="nodeNewValue">要更新的节点新值</param>
     /// <returns>更新成功返回true，失败返回false</returns>
     public static bool updateXmlNodeByXpathAndAttributeValue(string xmlFileName, string xpath, string xmlAttributeName, string value, string nodeNewValue) 
     {
         bool isSuccess = false;//是否成功操作
         XmlDocument xmlDoc = new XmlDocument();
         try
         {
             xmlDoc.Load(xmlFileName); //加载XML文档
             ...
             if (xmlNodes != null)
             {
                 foreach (XmlNode node in xmlNodes)
                 {
                     ...
                     if (node.InnerText.ToLower().Split(',')[0].Equals(nodeNewValue.ToLower().Split(',')[0]))
                     {
     ...
     /// <returns>成功返回true,失败返回false</returns>
     public static bool DeleteXmlNodeByAttributeAndXPath(string xmlFileName, string xpath, string xmlAttributeName,string xmlAttributeValue)
     {
         bool isSuccess = false;
         ...
         try
         {
             ...
             if (xmlNodes != null)
             {
                 foreach(XmlNode node in xmlNodes)
                 {
                     bool isNeedDeleteNode = false;
                 //遍历xpath节点中的所有属性
                  foreach (XmlAttribute attribute in node.Attributes)
                 {
                     if (attribute.Name.ToLower() == xmlAttributeName.ToLower() && attribute.Value.ToLower() == xmlAttributeValue.ToLower())
                     {
                         //节点中存在此属性
                        
                         isNeedDeleteNode = true;
