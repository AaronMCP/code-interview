...
{
    public class SocketClientNoMLLP : IClient
    {
        public const string DEVICE_NAME = "NO_MLLP_ONE_SESSION_PER_CONNECTION_CLIENT";

        public readonly SocketClientConfig Config;
        public SocketClientNoMLLP(SocketClientConfig cfg)
        {
            Config = cfg;

            if (Config == null)
                throw new ArgumentNullException("The constructor of the class SocketClientNoMLLP does not accept null in the cfg parameter.");
        }
        public SocketClientNoMLLP()
