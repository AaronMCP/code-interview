<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1" xmlns:ris="urn:hl7-org:v2xml" xmlns:csb="http://www.carestream.com/csbroker">
  ...
  <xsl:template name="ORDER_SIU">   
	...
	<xsl:choose>
              <xsl:when test="../MSH/FIELD.9/FIELD_ITEM/COMPONENT.2 = 'S12'">
                <xsl:text>Y</xsl:text>
              </xsl:when>
              <xsl:when test="../MSH/FIELD.9/FIELD_ITEM/COMPONENT.2 = 'S13' ">
                <xsl:text>Y</xsl:text>
              </xsl:when>
              <xsl:when test="../MSH/FIELD.9/FIELD_ITEM/COMPONENT.2 = 'S15' ">
                <xsl:text>N</xsl:text>
              </xsl:when>
            </xsl:choose>
        </xsl:element>
        ...
        <xsl:element name="csb:ORDER_EXAM_DT">
          <xsl:value-of select="../AIS/FIELD.4/FIELD_ITEM/COMPONENT.1"></xsl:value-of>
        </xsl:element>
