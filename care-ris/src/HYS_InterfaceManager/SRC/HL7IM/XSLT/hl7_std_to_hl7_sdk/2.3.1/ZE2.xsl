<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
  ...
  <ZE2>
     <FIELD.1>
         <xsl:if test="count(@SetID-ZE2 ) > 0">
             <FIELD_ITEM><xsl:value-of select="@SetID-ZE2"/></FIELD_ITEM>
         </xsl:if>
     </FIELD.1>
     
     <FIELD.2>
         <xsl:for-each select="XRayTubeVoltage">
             <xsl:call-template name="CQ"></xsl:call-template>
         </xsl:for-each>
     </FIELD.2>   
     
     <FIELD.3>
         <xsl:for-each select="XRayTubeElectricCurrent">
              <xsl:call-template name="CQ"></xsl:call-template>
         </xsl:for-each>
     </FIELD.3>   
