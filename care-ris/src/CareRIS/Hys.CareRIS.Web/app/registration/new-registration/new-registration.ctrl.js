newregistrationModule.controller('NewRegistratController', ['$log', '$scope', '$rootScope', 'registrationService', 'loginContext',
    'enums', '$translate', '$filter', 'registrationUtil', '$timeout', 'risInvalidFocus', 'application', '$state', 'configurationService', '$q', 'commonMessageHub','openDialog',
    function ($log, $scope, $rootScope, registrationService, loginContext,
        enums, $translate, $filter, registrationUtil, $timeout, risInvalidFocus, application, $state, configurationService, $q, commonMessageHub,openDialog) {
        'use strict';
        $log.debug('NewRegistratController.ctor()...');
        ...
        var initConfigurationData = function () {
            ...
            if (configurationData.genderList)
            {
        ...
        var cancel = function () {
            ...
            openDialog.openIconDialogOkCancel(openDialog.NotifyMessageType.Warn, $translate.instant('Alert'), '输入信息未保存，确定离开吗？', function () {
                if (requisitionFiles && requisitionFiles.length > 0) {
       ...
       $scope.popoverProcedureHistoryContainer = function () {
           $scope.isShowProcedureHistoryContainer = true;
        ...
        $scope.$on('$destroy', function () {
            // not saved ,should unlock slice
            if (!$scope.isSaved) {
                unlockTimeSlice(bookingSlice);
            }
        });
