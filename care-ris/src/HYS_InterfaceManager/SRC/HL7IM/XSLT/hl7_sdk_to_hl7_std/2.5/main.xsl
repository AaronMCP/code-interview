<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1" xmlns:ris="urn:hl7-org:v2xml" xmlns:csb="http://www.carestream.com/csbroker">
  ...
  <xsl:template match="/">
    ...
    <Message>
      ...
      <xsl:copy-of select="//Body/following-sibling::*"></xsl:copy-of>
    </Message>
  </xsl:template>
          ...
          <xsl:for-each select="MSH/following-sibling::*">
            <xsl:choose>
              ...
              <xsl:when test="name() = 'QPD'">
                <xsl:choose>
                  <xsl:when test="../MSH/FIELD.9/FIELD_ITEM/COMPONENT.1='QBP' and ../MSH/FIELD.9/FIELD_ITEM/COMPONENT.2='Q23'">
                    <xsl:call-template name="QPD_1"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:call-template name="QPD_2"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:when>
