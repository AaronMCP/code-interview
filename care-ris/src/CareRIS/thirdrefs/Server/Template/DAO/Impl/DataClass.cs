...
namespace SelfReportData
{
    public sealed class DataClass : IDisposable
    {
        ...
        //protected static log4net.ILog logger = log4net.LogManager.GetLogger("Webservice");  
        //constructor
        private DataClass()
        {
        ...
        static DataClass()
        {
            ...
            try
            {
                ...
                string strSQL = string.Format("select * from tdictionaryvalue");
                ...
                strSQL = string.Format("select * from  tUser");
                cmd.CommandText = strSQL;
                sda.SelectCommand = cmd;
                sda.Fill(ds, "user");
                ...
                strSQL = string.Format("select * from tDomainList");
                cmd.CommandText = strSQL;
                sda.SelectCommand = cmd;
        ...
        public static DataTable GetPrintBarorNoteTemplate()
        {
            SqlConnection conn = new SqlConnection();
            try
            {
                ...
                cmd.CommandText = string.Format("select * from tPrintTemplate where Domain='{0}'", domain);
        ...
        public static DataTable GetPrintTemplate()
        {
            ...
            if (_dtPrintTemplate == null)
            {
                ...
                try
                {
                    ...
                    string strSQL = string.Format("select * from  tPrintTemplate where type=3");
                    cmd.CommandText = strSQL;
                    SqlDataAdapter sda = new SqlDataAdapter();
                    sda.SelectCommand = cmd;
                    DataSet ds = new DataSet();
        ...
        public static bool InitalFtpParams()
        {
            bool bResult = false;
            SqlConnection conn = new SqlConnection();
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = conn;
                string strGetFTPSQL = string.Format("select * from tDomainlist where domain='{0}' ", ConfigurationManager.AppSettings["Domain"]);
        ...
        public static DataTable GetReportFileDt(string strReportGUID)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = conn;
                string strsql = string.Format("select * from tReportFile where ReportGuid='{0}'", strReportGUID);
                cmd.CommandText = strsql;
                SqlDataAdapter sda = new SqlDataAdapter();
                sda.SelectCommand = cmd;
                DataSet ds = new DataSet();
