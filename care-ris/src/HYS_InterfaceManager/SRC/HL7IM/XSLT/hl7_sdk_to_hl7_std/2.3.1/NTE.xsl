<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
  ...
  <NTE>
         <xsl:if test="FIELD.1/FIELD_ITEM != ''">
			 <xsl:attribute name="SetID-NTE"><xsl:value-of select="FIELD.1/FIELD_ITEM"/></xsl:attribute>
	     </xsl:if>
	     ...
	     <xsl:if test="FIELD.3/FIELD_ITEM != ''">
             <xsl:for-each select="FIELD.3/FIELD_ITEM">
                 <xsl:element name="ObservationIdentifier"><xsl:value-of select = "." /></xsl:element>
             </xsl:for-each>
         </xsl:if>
