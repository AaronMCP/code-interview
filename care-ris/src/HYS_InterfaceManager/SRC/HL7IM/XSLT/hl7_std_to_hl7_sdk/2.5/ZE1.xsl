<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
  ...
  <ZE1>
     ...
     <FIELD.5>
         <xsl:for-each select="MedicalAddition">
             <xsl:call-template name="CE"></xsl:call-template>
         </xsl:for-each>
     </FIELD.5>
     ...
     <FIELD.8>
         <xsl:if test="count(@UseCommodityClass ) > 0">
             <FIELD_ITEM><xsl:value-of select="@UseCommodityClass"/></FIELD_ITEM>
         </xsl:if>
     </FIELD.8>
     
     <FIELD.9>
         <xsl:for-each select="UseCommodity">
